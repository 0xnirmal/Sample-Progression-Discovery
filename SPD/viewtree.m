function node_positions = viewtree_arch(tree_adj)
warning off;
% tree_adj is undirected
adj = triu(tree_adj,1); adj = adj + adj';
fprintf('Computing tree layout, please wait patiently, if the number of samples is large ... \n    Total number of nodes: %5d\n    Visualized nodes: %5d',size(adj,1),0);
...
fprintf('\b\b\b\b\b%5d',sum(position_assigned_flag==1));    
        
% determin the nodes location of each node in the side chains
for i=1:length(side_chains)
