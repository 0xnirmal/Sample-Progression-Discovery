...
%                 repel_force = sum(repel_vector./repmat(sqrt(sum(repel_vector.^2,1)).^5,2,1),2);
                repel_force = sum(repel_vector./max(repmat(sqrt(sum(repel_vector.^2,1)).^5,2,1),1e-10),2);
...
node_positions = node_positions - repmat(max(node_positions,[],2)/2+min(node_positions,[],2)/2,1,size(node_positions,2));
